...
class FileFactory
{
     ...
     * @return \MacropaySolutions\KernelDev\Http\Testing\File
     */
...
//        return tap(new File($name, tmpfile()), function ($file) use ($kilobytes, $mimeType) {
        return tap(\di(File::class, [$name, tmpfile()]), function ($file) use ($kilobytes, $mimeType) {
...
//        return tap(new File($name, $tmpfile), function ($file) use ($tmpfile) {
        return tap(\di(File::class, [$name, $tmpfile]), function ($file) use ($tmpfile) {
