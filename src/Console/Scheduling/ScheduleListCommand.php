...
class ScheduleListCommand extends Command
{
    ...
    private function listEvent($event, $terminalWidth, $expressionSpacing, $repeatExpressionSpacing, $timezone)
    {
        ...
        if (!$this->output->isVerbose()) {
            $command = str_replace([Application::phpBinary(), Application::runBinary()], [
                'php',
                preg_replace("#['\"]#", '', Application::runBinary()),
            ], $command);
        }
        ...
        $command = preg_replace("#(php run [\w\-:]+) (.+)#", '$1 <fg=yellow;options=bold>$2</>', $command);
