...
trait RunsInParallel
{
    ...
    public function execute(): int
    {
        ...
        $this->forEachProcess(function () {
            \app(\MacropaySolutions\KernelDev\Testing\ParallelTesting::class)->callSetUpProcessCallbacks();
        });
        ...
        try {
            ...
            $this->forEachProcess(function () {
                \app(\MacropaySolutions\KernelDev\Testing\ParallelTesting::class)->callTearDownProcessCallbacks();
            });
        }
    ...
    protected function forEachProcess($callback)
    {
        ...
        collect(range(1, $processes))->each(function ($token) use ($callback) {
            tap($this->createApplication(), function ($app) use ($callback, $token) {
                \app(\MacropaySolutions\KernelDev\Testing\ParallelTesting::class)->resolveTokenUsing(fn() => $token);
    ...
    protected function createApplication()
    {
        $applicationResolver = static::$applicationResolver ?: function () {
            if (trait_exists(\Tests\CreatesApplication::class)) {
                ...
                file_exists($path = getcwd() . '/.framework/app.php')
            ) {
                $app = require $path;
