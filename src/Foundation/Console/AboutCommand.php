...
class AboutCommand extends Command
{
        ...
        collect(static::$data)
            ->map(fn($items) => collect($items)
                ->map(function ($value) {
                    ...
                    if (is_string($value)) {
                        $value = $this->app->make($value);
                    }

                    return collect($this->app->call($value))
    ...
    protected function gatherApplicationInformation()
    {
        ...
        static::addToSection('Environment', fn() => [
            ...
            'Framework Version' => $this->app->version(),
        ...
        static::addToSection('Cache', fn() => [
            'Config' => static::format($this->app->configurationIsCached(), console: $formatCachedStatus),
            'Events' => static::format($this->app->eventsAreCached(), console: $formatCachedStatus),
            'Routes' => static::format($this->app->routesAreCached(), console: $formatCachedStatus),
            'Views' => static::format(
                $this->hasPhpFiles($this->app->storagePath('framework/views')),
